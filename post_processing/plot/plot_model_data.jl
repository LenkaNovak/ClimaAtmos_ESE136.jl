...
function png_end(sol, var_name::Symbol; output_dir = ".", clims = nothing, level = 1)
    var = get_var(sol.u[end], Val(var_name))
...
function mp4(sol, var_name::Symbol; output_dir = ".", clims = nothing, level = 1)
    clims = isnothing(clims) ? map(x -> x * std( parent(get_var(sol.u[end], Val(var_name)))) * 2 , (-1,1) )  : clims
    anim = Plots.@animate for (i, u) in enumerate(sol.u) # note: sol.u = solution vector here, not zonal wind
        Plots.plot(get_var(u, Val(var_name)), clims = clims, title = string(var_name) *" "* string(sol.t[i]/86400), level = level, right_margin = 10Plots.mm)
    end
    Plots.mp4(anim, joinpath(output_dir, "anim_"*string(var_name)*".mp4"), fps = 10)
end
get_var(u, ::Val{:zonal_wind}) = Geometry.UVVector.(u.c.uₕ).components.data.:1
get_var(u, ::Val{:meridional_wind}) = Geometry.UVVector.(u.c.uₕ).components.data.:2
get_var(u, ::Val{:total_energy}) = u.c.ρe_tot
